...
static void supla_connection_on_register_result(supla_dev_t                   *dev,
                                                TSD_SuplaRegisterDeviceResult *regdev_res)
{
    ...
    case SUPLA_RESULTCODE_TRUE: {
        ...
        if (dev->supla_config.activity_timeout != regdev_res->activity_timeout) {
            supla_log(LOG_DEBUG, "Setting activity timeout to %ds",
                      dev->supla_config.activity_timeout);
...
static void supla_dev_on_set_channel_value(supla_dev_t *dev, TSD_SuplaChannelNewValue *new_value)
{
    ...
    srpc_ds_async_set_channel_result(dev->srpc, new_value->ChannelNumber, new_value->SenderID,
                                     success);
}

static void supla_dev_on_set_channel_group_value(supla_dev_t                   *dev,
                                                 TSD_SuplaChannelGroupNewValue *gr_value)
{
    TSD_SuplaChannelNewValue new_value;
    char                     success = 0;
    supla_log(LOG_DEBUG, "channel group[%d] set value request for ch[%d]", gr_value->GroupID,
              gr_value->ChannelNumber);
...
static void supla_dev_get_channel_state(supla_dev_t              *dev,
                                        TCSD_ChannelStateRequest *channel_state_request)
{
...
static void supla_dev_on_set_activity_timeout_result(supla_dev_t                        *dev,
                                                     TSDC_SuplaSetActivityTimeoutResult *res)
{
    dev->supla_config.activity_timeout = res->activity_timeout;
    supla_log(LOG_INFO, "Received activity timeout=%ds allowed[%d-%d]", res->activity_timeout,
              res->min, res->max);
}
...
static void supla_dev_on_calcfg_request(supla_dev_t *dev, TSD_DeviceCalCfgRequest *calcfg)
{
    ...
    supla_log(LOG_DEBUG,
              "Received calcfg from server: ch=%d cmd=%d auth=%d datatype=%d datasize=%d",
              calcfg->ChannelNumber, calcfg->Command, calcfg->SuperUserAuthorized, calcfg->DataType,
              calcfg->DataSize);
...
static void supla_dev_on_get_channel_functions_result(supla_dev_t          *dev,
                                                      TSD_ChannelFunctions *channel_functions)
{
    supla_channel_t *ch;
...
static void supla_dev_on_channel_config_finished(supla_dev_t               *dev,
                                                 TSD_ChannelConfigFinished *channel_config)
{
    supla_log(LOG_DEBUG, "Received channel %d config finished from server",
              channel_config->ChannelNumber);
}
...
static void supla_dev_on_set_device_config_result(supla_dev_t                *dev,
                                                  TSDS_SetDeviceConfigResult *result)
{
...
int supla_dev_set_config(supla_dev_t *dev, const struct supla_config *config)
{
    ...
    dev->supla_config.port = dev->supla_config.port ? dev->supla_config.port :
                             dev->supla_config.ssl  ? 2016 :
                                                      2015;
    dev->supla_config.activity_timeout =
        dev->supla_config.activity_timeout ? dev->supla_config.activity_timeout : 120;
    lck_unlock(dev->lck);
...
static int supla_dev_iterate_tick(supla_dev_t *dev)
{
    ...
    case SUPLA_DEV_STATE_INIT:
        ...
        supla_log(LOG_INFO, "[%s] Connecting to: %s:%d", dev->name, dev->supla_config.server,
                  dev->supla_config.port);
        supla_cloud_disconnect(&dev->cloud_link);
    ...
    case SUPLA_DEV_STATE_CONNECTED:
        if (difftime(sys_time.tv_sec, dev->reg_time.tv_sec) > 10) {
            ...
            supla_dev_set_connection_reset_cause(
                dev, SUPLA_LASTCONNECTIONRESETCAUSE_SERVER_CONNECTION_LOST);
            supla_dev_set_state(dev, SUPLA_DEV_STATE_INIT);
        }
        break;
    ...
    case SUPLA_DEV_STATE_ONLINE:
        ...
        if (supla_connection_ping(dev) == SUPLA_RESULT_FALSE) {
            supla_dev_set_connection_reset_cause(dev,
